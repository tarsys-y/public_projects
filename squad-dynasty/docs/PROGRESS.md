...
## Eixo 1 — Onboarding & Retenção (jul/2026)

- **Custos zerados (período de teste)**: pacotes, evolução, taxa de mercado e entrada do draft a custo 0; saldo inicial generoso. Economia real fica pra depois.
- **Time inicial "Master Liga FC"**: 18 jogadores clássicos da Master Liga do PES (Ivarov, Minanda, Castolo, Ordaz etc., posições/nacionalidades verificadas), substituindo a coleção demo Flamengo+Palmeiras.
- **Primeiro login**: nome do treinador, nome do time e escudo (12 paletas via `MonogramCrest`) antes de qualquer coisa; auto-monta o melhor 11 disponível ao confirmar.
- **Tela pré-jogo**: escudos+nomes+overall+escalação+banco dos dois times antes de "Começar partida" (amistoso) e "Jogar ao vivo" (carreira).
- **Indicador de próxima ação** na Home (montar time → 1ª partida → 1º pacote → 1ª carreira).
- **Missões de início** (5, one-time) e **recompensa de login diário** (streak de 7 dias) na Home.
