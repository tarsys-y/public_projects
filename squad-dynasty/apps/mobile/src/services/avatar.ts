...
/* ------------------------------------------------------------------ *
 * Adaptador LEGADO (até o render big-head da próxima fase): converte a
 * aparência nova para o formato que o PlayerAvatar atual desenha.
 * ------------------------------------------------------------------ */
...
export interface FaceFeatures {
  ...
  skinShade: string;
  hairColor: string;
  hairStyle: 'bald' | 'buzz' | 'short' | 'curly' | 'long' | 'topknot';
  facialHair: 'none' | 'stubble' | 'mustache' | 'beard';
}

const SKIN_HEX: Array<{ skin: string; shade: string }> = [
  ...
  { skin: '#54301a', shade: '#2f1a0d' },
  { skin: '#3f2413', shade: '#211107' },
];
...
const LEGACY_STYLE: Record<HairStyle, FaceFeatures['hairStyle']> = {
  bald: 'bald',
  balding: 'bald',
  buzz: 'buzz',
  'buzz-fade': 'buzz',
...
const LEGACY_BEARD: Record<Beard, FaceFeatures['facialHair']> = {
  none: 'none',
  stubble: 'stubble',
  goatee: 'mustache',
  circle: 'mustache',
...
export function faceFeatures(playerId: string): FaceFeatures {
  const a = resolveAppearance(playerId);
  const tone = SKIN_HEX[a.skinTone] ?? SKIN_HEX[2]!;
  return {
    skin: tone.skin,
    skinShade: tone.shade,
    hairColor: HAIR_HEX[a.hair.color],
    hairStyle: LEGACY_STYLE[a.hair.style],
    facialHair: LEGACY_BEARD[a.beard],
  };
}
