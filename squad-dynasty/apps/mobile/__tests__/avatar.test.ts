...
describe('avatar (artes das cartas)', () => {
  it('mesmo jogador ⇒ mesma cara, sempre', () => {
    expect(resolveAppearance('qualquer-um')).toEqual(resolveAppearance('qualquer-um'));
  });

  it('jogadores diferentes variam nas feições (sem cara clonada em massa)', () => {
    const sample = Array.from({ length: 200 }, (_, i) => proceduralAppearance(`player-${i}`));
    expect(new Set(sample.map((f) => f.hair.style)).size).toBeGreaterThanOrEqual(6);
    expect(new Set(sample.map((f) => f.skinTone)).size).toBeGreaterThanOrEqual(4);
    expect(new Set(sample.map((f) => f.hair.color)).size).toBeGreaterThanOrEqual(4);
    expect(new Set(sample.map((f) => f.beard)).size).toBeGreaterThanOrEqual(4);
    expect(new Set(sample.map((f) => f.faceShape)).size).toBe(3);
  });
